...
export default function HomePageSinglePage() {
  ...
  const normalizedTemplates = Array.isArray(allTemplates) 
    ...
    : (allTemplates && typeof allTemplates === "object" && "templates" in allTemplates && Array.isArray((allTemplates as any).templates)
        ? (allTemplates as any).templates
        : []);
      ...
      <section>
        <div className="flex overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-muted-foreground/20 scrollbar-track-transparent">
          <div className="flex gap-2">
            ...
            {!isCategoriesLoading && (Array.isArray(categories) ? categories : []).map((category: any) => (
              <Button
                key={category.id}
