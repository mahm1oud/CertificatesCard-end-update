...
const safeLayersForDisplay = layersForDisplay.map(layer => ({
  ...layer,
  position: 'position' in layer && layer.position ? layer.position : { x: 0, y: 0 }
}));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="lg:col-span-1">
          <Card>
            ...
            <CardContent>
                <LayersPanelSimple
                      fields={safeLayersForDisplay}
                      activeLayerId={selectedFieldId} // ✅ الاسم الصحيح كما هو معرف في props
                      onLayerSelect={setSelectedFieldId}
                      onLayerVisibilityToggle={handleLayerVisibilityToggle}
                      onMoveLayerUp={(id: number) => {
                        const index = layersForDisplay.findIndex(layer => layer.id === id);
                        if (index > 0) handleLayerOrderChange(index, index - 1);
                      }}
                      onMoveLayerDown={(id: number) => {
                        const index = layersForDisplay.findIndex(layer => layer.id === id);
                        if (index < layersForDisplay.length - 1) handleLayerOrderChange(index, index + 1);
                      }}
                      onFieldsUpdate={(updatedFields: any[]) => setFields(updatedFields)}
                      onLayerClick={(id: number) => setSelectedFieldId(id)}
