...
export function CopyFieldsDialog({ 
  ...
  const copyFieldsMutation = useMutation({
    mutationFn: () => {
      const targetId = parseInt(targetTemplateId);
  if (isNaN(targetId)) throw new Error("Target template ID is invalid");
  return apiRequest(
    "POST",
    `/api/templates/${sourceTemplateId}/copy-fields`,
    { targetTemplateId: targetId, fieldIds: selectedFields }
  );
},
    onSuccess: () => {
      toast({
