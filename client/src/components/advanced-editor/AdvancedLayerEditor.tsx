...
import { Stage, Layer, Image as KonvaImage, Group, Text, Rect, Transformer, Line, Circle, Star } from 'react-konva';
import {
  ZoomIn, 
  ZoomOut, 
  RotateCcw, 
...
interface FieldType {
  ...
  label?: string;
  labelAr?: string;
  type: 'text' | 'image' | 'template' |'shape' | string;
  ...
  gradient?: {
    enabled: boolean;
    type: 'linear' | 'radial';
    colors: string[]; // أمثلة: ['#fff', '#000']
    direction?: 'horizontal' | 'vertical'; // للخطية فقط
  };
...
export const AdvancedLayerEditor: React.FC<AdvancedLayerEditorProps> = ({
  ...
  const renderShapeField = useCallback((field: FieldType, index: number) => {
  if (field.visible === false) return null;

  const { shapeType = 'rect', stroke = '#333', strokeWidth = 2, opacity = 1, size = { width: 100, height: 100 } } = field.style || {};
const shadow = field.style?.shadow;
...
const gradient = field.style?.gradient;
const fill = gradient?.enabled
  ? {
      start: { x: 0, y: 0 },
      end: gradient.direction === 'vertical' ? { x: 0, y: size.height } : { x: size.width, y: 0 },
      colorStops: gradient.colors.flatMap((color, i) => [i / (gradient.colors.length - 1), color])
    }
  : field.style?.fill || '#3498db';
...
shadowOffset: { x: shadowOffsetX, y: shadowOffsetY },
    ...
    onTap: () => handleFieldSelect(field.id, false),
    onDblClick: () => handleFieldDoubleClick(field.id),
    onDblTap: () => handleFieldDoubleClick(field.id),
    onDragEnd: (e: any) => {
      const pos = snapToGrid({ x: e.target.x(), y: e.target.y() });
      const updatedFields = fields.map(f =>
        f.id === field.id ? { ...f, position: pos } : f
      );
      commitChanges(updatedFields);
    }
  };
  ...
  switch (shapeType) {
    case 'circle':
      return <Circle {...commonProps} radius={Math.min(size.width, size.height) / 2} />;
    case 'triangle':
      return (
        <Line
          {...commonProps}
    ...
    default:
      ...
      fillLinearGradientStartPoint={gradient?.enabled ? { x: 0, y: 0 } : undefined}
fillLinearGradientEndPoint={gradient?.enabled && gradient.direction === 'vertical'
  ? { x: 0, y: size.height }
  : gradient?.enabled
  ? { x: size.width, y: 0 }
  : undefined}
fillLinearGradientColorStops={gradient?.enabled ? gradient.colors.flatMap((color, i) => [i / (gradient.colors.length - 1), color]) : undefined}
fill={!gradient?.enabled ? fill : undefined}

      />;
  }
}, [fields, readOnly, handleFieldSelect, handleFieldDoubleClick, snapToGrid, commitChanges]);

const ElementsPanel = () => {
  const createShape = (shapeType: FieldType['style']['shapeType']) => {
    const newId = Math.max(0, ...fields.map(f => f.id)) + 1;

    const newField: FieldType = {
      id: newId,
      name: `${shapeType}_shape_${newId}`,
      type: 'shape',
  ...
  return (
    <ScrollArea className="h-[calc(100vh-200px)]">
      <div className="p-4 space-y-4">
        <h3 className="text-lg font-semibold">أضف شكلاً جديدًا</h3>
        <div className="grid grid-cols-2 gap-2">
          <Button variant="outline" onClick={() => createShape('rect')}>
            مستطيل
          </Button>
          <Button variant="outline" onClick={() => createShape('circle')}>
            دائرة
          </Button>
          <Button variant="outline" onClick={() => createShape('triangle')}>
            مثلث
          </Button>
          <Button variant="outline" onClick={() => createShape('line')}>
            خط
          </Button>
          <Button variant="outline" onClick={() => createShape('star')}>
            نجمة
          </Button>
        </div>
      </div>
    ...
    return (
      <ScrollArea className="h-[calc(100vh-200px)]">
        <div className="p-4 space-y-4">
          ...
          {/* خصائص الشكل - للأشكال فقط */}
{selectedField.type === 'shape' && (
  <>
    <FieldPropertyHint
      propertyType="color"
      currentValue={{
        fill: selectedField.style?.fill || '#3498db',
        stroke: selectedField.style?.stroke || '#2980b9',
        strokeWidth: selectedField.style?.strokeWidth || 2
      }}
      onChange={(value) => {
        updateFieldStyle('fill', value.fill);
        updateFieldStyle('stroke', value.stroke);
        updateFieldStyle('strokeWidth', value.strokeWidth);
      }}
      hint="تغيير لون الشكل، لون الحدود وسماكة الحدود"
    />

    <FieldPropertyHint
      propertyType="size"
      currentValue={{
        width: selectedField.style?.size?.width || 100,
        height: selectedField.style?.size?.height || 100
      }}
      onChange={(value) => {
        updateFieldStyle('size', {
          width: value.width,
          height: value.height
        });
      }}
      hint="تحديد أبعاد الشكل"
    />
  </>
)}
...
{selectedField.type === 'shape' && (
  <FieldPropertyHint
    propertyType="shadow"
    currentValue={{
      shadow: selectedField.style?.shadow || { enabled: false }
    }}
    onChange={(value) => updateFieldStyle('shadow', value.shadow)}
    hint="تخصيص ظل الشكل"
  />
)}
...
{selectedField.type === 'shape' && (
  <FieldPropertyHint
    ...
    currentValue={{
      gradient: selectedField.style?.gradient || {
        enabled: false,
        type: 'linear',
    ...
    onChange={(value) => updateFieldStyle('gradient', value.gradient)}
    hint="تفعيل وتخصيص التدرج اللوني للشكل"
  />
)}
            ...
            <Layer ref={mainLayerRef}>
              ...
              {sortedFields.map((field, index) => {
                  switch (field.type) {
                    ...
                    case 'shape':
                      return renderShapeField(field, index);
                    default:
                      return null;
                  }
                })}
