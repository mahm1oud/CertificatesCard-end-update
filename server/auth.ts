...
export function setupAuth(app: Express) {
  ...
  cookie: {
  //  secure: process.env.NODE_ENV === 'production', // فقط true في الإنتاج (https)
    secure:false,
    httpOnly: true,
    maxAge: 7 * 24 * 60 * 60 * 1000,
    sameSite: process.env.NODE_ENV === 'production' ? 'none' : 'lax'  // sameSite 'none' مع secure=true و https
  }
};
  ...
  function isStrategyEnabled(name: string) {
    // passport._strategy(name) throws if not found, so we use try/catch
    try {
