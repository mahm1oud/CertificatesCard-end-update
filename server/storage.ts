...
import {type User, type InsertUser} from "@shared/schema";
 import { categories, type Category, type InsertCategory,
...
export class DatabaseStorage implements IStorage {
        ...
        if (weddingCategory && eidCategory && ramadanCategory && graduationCategory && engagementCategory && certificatesCategory) {
          ...
          const fontsData: InsertFont[] = [
            ...
            },
            {
              ...
              type: 'custom',
              active: true,
              url: 'https://fonts.googleapis.com/css2?family=Noto+Naskh+Arabic:wght@400..700&display=swap', // استبدل برابط الخط الصحيح
              isRtl: true,
              displayOrder: 5
            }
          ];
  ...
  async getAllCategories(options: { active?: boolean } = {}): Promise<Category[]> {
    ...
    if (active !== undefined) {
      query = (query as any).where(eq(categories.active, active));
    }
  ...
  async getAllTemplates(options: { active?: boolean; limit?: number; offset?: number; search?: string } = {}): Promise<{ templates: Template[]; total: number }> {
    ...
    if (active !== undefined) {
      query = (query as any).where(eq(templates.active, active));
    }
  ...
  async getUserCertificates(userId: number, options: { limit?: number; offset?: number; type?: string } = {}): Promise<{ certificates: Certificate[]; total: number }> {
    ...
    if (type) {
      query = (query as any).where(eq(certificates.certificateType, type));
    }
  ...
  async getBatchItems(batchId: number, options: { limit?: number; offset?: number; status?: string } = {}): Promise<{ items: CertificateBatchItem[]; total: number }> {
    ...
    if (status) {
      query = (query as any).where(eq(certificateBatchItems.status, status));
    }
  ...
  async getAllFonts(options: { active?: boolean } = {}): Promise<Font[]> {
    ...
    if (active !== undefined) {
      query = (query as any).where(eq(fonts.active, active));
    }
  ...
  async getAllCards(options: { limit?: number; offset?: number; search?: string; status?: string } = {}): Promise<{ cards: Card[]; total: number }> {
    try {
      ...
      if (status) {
        query = (query as any).where(eq(cards.status, status));
        countQuery = (countQuery as any).where(eq(cards.status, status));
      }
      
      if (search) {
        query = (query as any).where(sql`LOWER(cards.title) LIKE ${`%${search.toLowerCase()}%`}`);
        countQuery = (countQuery as any).where(sql`LOWER(cards.title) LIKE ${`%${search.toLowerCase()}%`}`);
      }
      ...
      query = (query as any).orderBy(desc(cards.createdAt));
  ...
  async getAllCertificates(options: { limit?: number; offset?: number; search?: string; type?: string } = {}): Promise<{ certificates: Certificate[]; total: number }> {
    try {
      ...
      if (type) {
        query = (query as any).where(eq(certificates.certificateType, type));
        countQuery = (countQuery as any).where(eq(certificates.certificateType, type));
      }
      
      if (search) {
        query = (query as any).where(sql`LOWER(certificates.title) LIKE ${`%${search.toLowerCase()}%`}`);
        countQuery = (countQuery as any).where(sql`LOWER(certificates.title) LIKE ${`%${search.toLowerCase()}%`}`);
      }
      ...
      query = (query as any).orderBy(desc(certificates.createdAt));
  ...
  async getUserSignatures(userId: number, type?: string): Promise<UserSignature[]> {
    try {
      ...
      const signaturesList = await query.orderBy(desc(userSignatures.id));
