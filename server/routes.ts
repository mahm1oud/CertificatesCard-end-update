...
// Extend express-session types to include userPreferences
...
export async function registerRoutes(app: Express): Promise<Server> {
        ...
        try {
          ...
          try {
            ...
            try {
              ...
              let templateSettings = (template.settings || {}) as { width?: string | number, height?: string | number, paperSize?: string, orientation?: string };
              
              // إعدادات افتراضية إذا لم تكن موجودة
              const outputWidth = templateSettings?.width ? parseInt(templateSettings.width as string) : 1200;
              const outputHeight = templateSettings?.height ? parseInt(templateSettings.height as string) : 1600;
          ...
          return res.status(500).json({ 
            ...
            details: typeof imageError === 'object' && imageError !== null ? (imageError as any).message : String(imageError)
          });
        }
      } catch (dbError) {
      ...
      try {
        ...
        generatedImagePath = await import('./optimized-image-generator').then(({ generateOptimizedCardImage }) => {
          return generateOptimizedCardImage({
            ...
            formData: card.formData as Record<string, any>,
        ...
        generatedImagePath = await generateCardImage(
          { ...template, templateFields: templateFields as any[] }, 
          card.formData, 
          quality
        );
      }
  ...
  app.post("/api/certificates/generate", async (req, res) => {
    try {
      ...
      const certificateType = formData.certificateType || 'appreciation';
  ...
  app.post("/api/certificate-batches", isAuthenticated, upload.single('file'), async (req, res) => {
    try {
      ...
      const batch = await storage.createCertificateBatch({
        userId: req.user.id,
        templateId: template.id,
        name: title || `مجموعة شهادات ${(new Date()).toLocaleDateString('ar-SA')}`,
        status: 'pending',
        totalItems: 0,
        processedItems: 0,
  ...
  app.put("/api/user/profile", isAuthenticated, async (req, res) => {
    try {
      ...
      const updatedUser = await storage.updateUser(req.user.id, { fullName: name, email });
  ...
  app.put("/api/admin/users/:id", isAdmin, async (req, res) => {
    try {
      ...
      const updatedUser = await storage.updateUser(parseInt(id), { 
        role, active, fullName: name, email 
      });
  ...
  app.get('/api/admin/templates/:templateId/layout', isAuthenticated, isAdmin, async (req, res) => {
    try {
      ...
      const layoutData = (template.settings as any)?.layoutData || [];
      res.json(layoutData);
    } catch (error) {
