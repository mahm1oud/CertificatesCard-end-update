...
router.patch('/:id/update-design', async (req: Request, res: Response) => {
  try {
    ...
    let updatedFormData: { [key: string]: any } = { ...(typeof card.formData === 'object' && card.formData !== null ? card.formData : {}) };
...
router.post('/:id/download', async (req: Request, res: Response) => {
  try {
    ...
    const formData: { [key: string]: any } = card.formData || {};
    
    // إعدادات الإخراج
    type TemplateSettings = { width?: string | number; height?: string | number; [key: string]: any };
    const templateSettings: TemplateSettings = template.settings || {};
